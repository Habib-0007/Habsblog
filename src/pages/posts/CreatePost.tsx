...
//     const formData = new FormData();
//     formData.append('title', title);
//     formData.append('content', content); // content already contains base64 image data
...
//       },
//       onError: (error) => {
//         toast.error(`Error: ${error.message || 'Failed to save post'}`);
//       }
//     });
//   };
...
//               value={content}
//               onChange={setContent}
//               placeholder="Write your post content here... Click the image icon to add images."
//               error={errors.content}
//             />
//             <p className="text-sm text-muted-foreground mt-1">
//               You can add images directly in your content using the image button in the toolbar.
//             </p>
//           </div>
...
import MarkdownEditor from '../../components/posts/MarkdownEditor';
import { useCreatePost } from '../../hooks/usePosts';
import { getImage } from '../../utils/imageUpload';
...
const CreatePost = () => {
  ...
  // Function to process content before submitting
  const processContent = (markdownContent: string): string => {
    // Replace our custom image:// protocol with actual base64 data
    const imageRegex = /!\[(.*?)\]\(image:\/\/(.*?)\)/g;

    return markdownContent.replace(imageRegex, (match, altText, imageId) => {
      const storedImage = getImage(imageId);
      if (storedImage) {
  ...
  const handleSubmit = (
    ...
    // Process content to replace image IDs with actual base64 data
      ...
      <Card className="md:p-6">
        ...
        <form className="space-y-6">
          ...
          <div>
            ...
            />
            <p className="text-sm text-muted-foreground mt-1">
              You can add images directly in your content using the image button
              in the toolbar.
            </p>
          </div>
